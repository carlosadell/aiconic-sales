...
// Open slots for the next few days in one call, grouped by day, in the given
// time zone. Days with no open slots are left out. Returns
// [{ date: "YYYY-MM-DD", slots: [iso, ...] }, ...] soonest first.
async function getFreeSlotsAhead(calendarId, timezone, days = 14) {
  let today;
  try { today = new Date().toLocaleDateString("en-CA", { timeZone: timezone || "UTC" }); }
  catch (_) { today = new Date().toISOString().slice(0, 10); }
  const base = Date.parse(`${today}T00:00:00Z`);
  const dayMs = 24 * 3600 * 1000;
  const params = new URLSearchParams({
    startDate: String(base - dayMs),
    endDate: String(base + (days + 1) * dayMs),
  });
  if (timezone) params.set("timezone", timezone);
  const data = await ghlFetch(`/calendars/${calendarId}/free-slots?${params.toString()}`, {}, 4, CALENDAR_VERSION);
  const last = new Date(base + (days - 1) * dayMs).toISOString().slice(0, 10);
  const now = Date.now();
  return Object.keys(data || {})
    .filter((k) => /^\d{4}-\d{2}-\d{2}$/.test(k) && k >= today && k <= last)
    .sort()
    .map((k) => ({ date: k, slots: ((data[k] && data[k].slots) || []).filter((iso) => Date.parse(iso) > now) }))
